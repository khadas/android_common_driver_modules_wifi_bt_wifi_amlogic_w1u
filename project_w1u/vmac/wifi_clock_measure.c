...
unsigned int clk_msr_calc(unsigned int clk_mux, unsigned int exp_freq)
{
    ...
    AML_PRINT_LOG_INFO("exp_freq = %d(MHz)\n", exp_freq / 1000000);
    AML_PRINT_LOG_INFO("measured_feq = %d(MHz)\n", measured_feq / 1000000);

    if (measured_val == 0xFFFFF) {
        AML_PRINT_LOG_INFO("measured_val is 20 bits max\n");
        return 0;
    } else if ((measured_feq < (exp_freq - delt)) || (measured_feq > (exp_freq + delt))) {
        AML_PRINT_LOG_INFO("clock measure failed, clock id : %d\n", clk_mux);
        return 0;
    } else {
        return 1;
    }
...
void clk_msr_other(CLOCK_MUX clk_mux)
{
    ...
    switch (clk_mux) {
        ...
        case CTS_CLK_VIT_POS:
            AML_PRINT_LOG_INFO("not measure now\n");
